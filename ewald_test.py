...
class Ewald:
    ...
    def ewaldG2_deriv(self, w, type, lattice):
        ...
        for R_pos in lattice:
            ...
            if type is 'xx':
                for j in range(0, self.j_max + 1):
                    if j == 0:
                        _sum += (1/np.math.factorial(j)) * (k/(2*self.E))**(2*j) * (-2*self.E**2*sp.special.expn(j, rho_E_sq))
                    else:
                        _sum += (1/np.math.factorial(j)) * (k/(2*self.E))**(2*j) * ((-2*self.E**2*sp.special.expn(j, rho_E_sq)) +
                        4 * rho[0]**2 * self.E**4 * sp.special.expn(j-1, rho_E_sq))
                ...
                for j in range(0, self.j_max + 1):
                    if j == 0:
                        ...
                        _sum += (1/np.math.factorial(j)) * (k/(2*self.E))**(2*j) * ((-2*self.E**2*sp.special.expn(j, rho_E_sq)) +
                        4 * rho[1]**2 * self.E**4 * sp.special.expn(j-1, rho_E_sq))
                _sum += 4/(n_rho**4)*rho[1]**2*(rho_E_sq + 1)*np.exp(-rho_E_sq)
            elif type is 'xy':
                for j in range(1, self.j_max + 1):
                        _sum += 4 * rho[0] * rho[1] * self.E**4 * sp.special.expn(j-1, rho_E_sq)
    ...
    def ewaldG1_deriv(self, w, type):
        ...
        for G_pos in self.reciprocal:
            if type is 'xx':
                _sum += -(self.q+G_pos)[0]**2 * (1./area) * (np.exp(1j*np.dot(self.q+G_pos, self.pos)) * np.exp((k**2 - np.linalg.norm(q+G_pos)**2)/(4*self.E**2)))/((self.q+G_pos)[0]**2 - k**2)
            elif type is 'yy':
                _sum += -(self.q+G_pos)[1]**2 * (1./area) * (np.exp(1j*np.dot(self.q+G_pos, self.pos)) * np.exp((k**2 - np.linalg.norm(q+G_pos)**2)/(4*self.E**2)))/((self.q+G_pos)[1]**2 - k**2)
            if type is 'xy':
                _sum += -(self.q+G_pos)[0]*(self.q+G_pos)[1] * (1./area) * (np.exp(1j*np.dot(self.q+G_pos, self.pos)) * np.exp((k**2 - np.linalg.norm(q+G_pos)**2)/(4*self.E**2)))/((self.q+G_pos)[0]*(self.q+G_pos)[1])
        return _sum
    ...
    def reducedDyadicLatticeSum(self, w):
        ...
        print(pre_factor, self.t2_deriv(w, 'xx'), self.t2_deriv(w, 'yy'))
        return [xx_comp, xy_comp, yy_comp]
...
def testLatticeSum(vector_1, vector_2, neighbour_range, q, w, pos):
    ...
    if np.linalg.norm(np.array(pos)) == 0:  # if you are the origin, then exclude it from the sum
        ...
        for i in range(1, 20):
            ewald_results.append(Ewald(2*np.pi/(a), 20, q, lattice, i, pos).monopolarSum(w))
        print("done")
...
def testDyadicSum(vector_1, vector_2, neighbour_range, q, w, pos=[0, 0]):
    ...
    if np.linalg.norm(np.array(pos)) == 0:  # if you are the origin, then exclude it from the sum
        ...
        for i in loop_range:
            ...
            ewald_results.append(Ewald(np.pi/a, 20, q, lattice, i, pos).reducedDyadicLatticeSum(wp))
            print(i**2-1)
        print("done")
    else:
        print("Sum including origin")
